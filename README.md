...
The notebook creates `cpdb_inputs/metadata.tsv` and `cpdb_inputs/counts_log1p.h5ad` from the processed object. The latter copies `adata.X`, not `adata.raw.X`. The upstream code log-transforms the supplied TPM-labelled matrix; it does not calculate TPM from raw counts. The exact original contents of `zhang_processed.h5ad` have not been inspected.

The recorded CellPhoneDB run uses **43,817 cells and 2,000 genes**, consistent with the upstream highly variable gene restriction. Interactions requiring excluded genes cannot be evaluated with that input. For a revised communication analysis, recover the broader log-transformed expression matrix from `.raw` if it is retained in the saved object, and rerun both Method 2 analyses and dependent plots. Preserve the existing outputs separately for comparison.
...
7. Separate the highly variable gene matrix used for embedding from the broader expression input needed to assess communication coverage. Inspect the saved object’s `.raw` before using it; in the upstream code `.raw` is assigned after log transformation, so it should not be log-transformed again. Rerun and compare results if the communication input is changed.
